const RajasthanCardData = [
  {
    title: "Jaipur",
    image: "/images/Rajasthan_img/Hawa_mahal.jpg",
    description: "A UNESCO World Heritage Site, Jaipur is dotted with numerous heritage sites that are unmissable."
  },
  {
    title: "Udaipur",
    image: "/images/Rajasthan_img/Udaipur_images.jpg",
    description: "The City of Lakes, Udaipur is about the royal palaces, man-made lakes, and the Aravalli Hills."
  },
  {
    title: "Jodhpur",
    image: "/images/Rajasthan_img/Jodhpur_image.jpg",
    description: "Home to the impressive Mehrangarh Fort and Umaid Bhavan, Jodhpur, the second-largest city in Rajasthan is a history lover's paradise."
  },
  {
    title: "Jaisalmer",
    image: "/images/Rajasthan_img/Jaisalmer_image.jpg",
    description: "The Golden City, Jaisalmer reflects the rich history of Rajasthan and offers experiences you cannot forget."
  },
  {
    title: "Pushkar",
    image: "/images/Rajasthan_img/PUSHKAR_image.jpg",
    description: "A religious destination with 52 ghats and one-of-its-kind Brahma Temple, Pushkar attracts many from around the world."
  }
];

const GujratCardData = [
  {
    title: "Ahmedabad",
    image: "/images/Rajasthan_img/Ahmedabad.jpg",
    description: "Ahmedabad is known for its rich culture and the Sabarmati Ashram."
  },
  {
    title: "Surat",
    image: "/images/Rajasthan_img/Surat.jpg",
    description: "Surat is famous for its diamond cutting and polishing and textiles."
  },
  {
    title: "Vadodara",
    image: "/images/Rajasthan_img/Vadodra.jpg",
    description: "Vadodara is home to several majestic palaces and museums."
  },
  {
    title: "Dwarka",
    image: "/images/Rajasthan_img/Dwarka.jpg",
    description: "Dwarka is known for the Dwarkadhish Temple and its significance in Hindu mythology."
  },
  {
    title: "Gir National Park",
    image: "/images/Rajasthan_img/Gir.jpg",
    description: "Gir National Park is the only home for Asiatic lions."
  }
];

const LadakhCardData = [
  {
    title: "Leh",
    image: "/images/Ladakh_img/Leh_Lake.jpg",
    description: "The capital city, Leh, has numerous attractions including Shanti Stupa and a beautiful market."
  },
  {
    title: "Nubra Valley",
    image: "/images/Ladakh_img/Nubra_Valley.jpg",
    description: "Home to an alpine cold desert, the 33-meter statue of Jampa, and Bactrian Camels, Nubra Valley is a must-see in Ladakh."
  },
  {
    title: "Zanskar",
    image: "/images/Ladakh_img/Zanskar.jpg",
    description: "An offbeat and lesser-visited side of Ladakh, Zanskar is primarily known for its Frozen River Trek."
  },
  {
    title: "Drass",
    image: "/images/Ladakh_img/Drass.jpg",
    description: "Called the Gateway to Ladakh, Drass is also known as the second coldest inhabited place in the world."
  },
  {
    title: "Tso Moriri",
    image: "/images/Ladakh_img/Tso_Moriri.jpg",
    description: "A serene lake in the Changthang region, surrounded by snow-capped mountains."
  }
];

const JammuAndKashmirCardData = [
  {
    title: "Srinagar",
    image: "/images/J&K_img/Srinagar.jpg",
    description: "A city to get a unique houseboat and shikara ride experience on Dal and Nigeen Lake."
  },
  {
    title: "Gulmarg",
    image: "/images/J&K_img/Gulmarg.jpg",
    description: "Gulmarg in Kashmir is a winter wonderland, making it the best skiing destination in India."
  },
  {
    title: "Pahalgam",
    image: "/images/J&K_img/Pahalgam.jpg",
    description: "Pahalgam in Kashmir is the starting point of the annual pilgrimage to the cave of Amarnath."
  },
  {
    title: "Sonamarg",
    image: "/images/J&K_img/Sonamarg.jpg",
    description: "Sonmarg, which means 'Meadow of Gold', is a picturesque hill station in the Kashmir Valley located in the Indian union territory of Jammu and Kashmir"
  },
  {
    title: "Jammu City",
    image: "/images/J&K_img/Jammu.jpg",
    description: "The winter capital, Jammu City is nestled against the backdrop of the Shivalik and Trikuta Ranges."
  }
];

const HimachalPradeshCardData = [
  {
    title: "Shimla",
    image: "/images/HimachalPradesh_img/Shimla.jpg",
    description: "Explore colonial architecture, hang out at The Mall & enjoy the gorgeous views greeting you everywhere. "
  },
  {
    title: "Kullu Manali",
    image: "/images/HimachalPradesh_img/Manali.jpg",
    description: "Manali is a top tourist attraction for its tourist sites like Hidimba Devi Temple, Old Manali, bazaar & a rich cultural heritage."
  },
  {
    title: "Dharamshala",
    image: "/images/HimachalPradesh_img/Dharamshala.jpg",
    description: "Dharamshalais a beautiful city located in the Kangra Valley, famous for its serene natural beauty and Tibetan culture."
  },
  {
    title: "Kasauli",
    image: "/images/HimachalPradesh_img/Kasauli.jpg",
    description: "A quaint town, Kasauli is an attractive mix of picturesque hiking trails, snow-clad mountains and lush landscape"
  },
  {
    title: "Dalhousie",
    image: "/images/HimachalPradesh_img/Dalhousie.jpg",
    description: "Dalhousieis a small hill station located in the Chambadistrict of this state, famous for its colonial charm and beautiful natural scenery."
  }
];

const UttarakhandCardData = [
  {
    title: "Nainital",
    image: "/images/Uttarakhand_img/Nainital.jpg",
    description: "Adorned with an eye-shaped lake and rolling green hills, Nainital is a popular destination in Uttarakhand."
  },
  {
    title: "Mussoorie",
    image: "/images/Uttarakhand_img/Mussoorie.jpg",
    description: "Called the Queen of the Hills, Mussoorie is one of the most popular tourist destinations in Uttarakhand."
  },
  {
    title: "Haridwar",
    image: "/images/Uttarakhand_img/Haridwar.jpg",
    description: "A spiritual destination dotted with numerous Hindu temples, Haridwar is the gateway of Garhwal Region."
  },
  {
    title: "Rishikesh",
    image: "/images/Uttarakhand_img/Rishikesh.jpg",
    description: "The Yoga Capital of India, Rishikesh and is a world famous yoga and adventure destination."
  },
  {
    title: "Jim Corbett National Park",
    image: "/images/Uttarakhand_img/Jim_Corbett.jpg",
    description: "A famous wildlife sanctuary known for its Bengal tigers and diverse flora and fauna."
  }
];

const SikkimCardData = [
  {
    title: "Gangtok",
    image: "/images/Sikkim_img/Gangtok.jpg",
    description: "It offers a little of everything: culture, nature, entertainment & Tibetan food. Visit monasteries & views of Mt. Khangchendzonga from Tashi View Point"
  },
  {
    title: "Pelling",
    image: "/images/Sikkim_img/Pelling.jpg",
    description: "Views of Mt. Khangchendzonga will leave you captivated. Waterfalls, breathtaking views and plenty of adventure opportunities."
  },
  {
    title: "Yuksom",
    image: "/images/Sikkim_img/Yuksom.jpg",
    description: "It's the origin of several treks & is blessed with exceptional beauty. You will find the oldest monastery here."
  },
  {
    title: "Namchi",
    image: "/images/Sikkim_img/Namchi.jpg",
    description: "Visit Namchi, because it looks like a portrait. Attend the Namchi Mahotsav in October. It’s also home to the 108 feet statue of Lord Shiva & 118 feet statue of Guru Padmasambhava."
  },
  {
    title: "Lachen",
    image: "/images/Sikkim_img/Lachen.jpg",
    description: "A small village which is called the twin sister of Lachung, Lachen is situated in North Sikkim. It is the base for Gurudongmar Lake, Chopta Valley and numerous treks."
  }
];

const WestBengalCardData = [
  {
    title: "Kolkata",
    image: "/images/WestBengal_img/Kolkata.jpg",
    description: "The cultural capital of India, Kolkata is famous for its colonial architecture and vibrant festivals."
  },
  {
    title: "Darjeeling",
    image: "/images/WestBengal_img/Darjeeling.jpg",
    description: "Known for its tea plantations and panoramic views of the Himalayas, Darjeeling is a popular hill station."
  },
  {
    title: "Sundarbans",
    image: "/images/WestBengal_img/Sundarbans.jpg",
    description: "The largest mangrove forest in the world, Sundarbans is home to the Royal Bengal Tigers."
  },
  {
    title: "Kalimpong",
    image: "/images/WestBengal_img/Kalimpong.jpg",
    description: "A serene hill station known for its colonial architecture, monasteries, and flower markets."
  },
  {
    title: "Siliguri",
    image: "/images/WestBengal_img/Siliguri.jpg",
    description: "Enveloped by the Himalayan mountains amplifying the scenic natural beauty all around, Siliguri is a picture-perfect holiday tourism place in West Bengal."
  }
];

const AssamCardData = [
  {
    title: "Guwahati",
    image: "/images/Assam_img/Guwahati.jpg",
    description: "The largest city in Assam, Guwahati is known for its ancient temples and the mighty Brahmaputra River."
  },
  {
    title: "Kaziranga National Park",
    image: "/images/Assam_img/Kaziranga.jpg",
    description: "A UNESCO World Heritage Site, Kaziranga National Park is famous for its population of the one-horned rhinoceros."
  },
  {
    title: "Majuli",
    image: "/images/Assam_img/Majuli.jpg",
    description: "The world's largest river island, Majuli is known for its unique culture and scenic beauty."
  },
  {
    title: "Sivasagar",
    image: "/images/Assam_img/Sivasagar.jpg",
    description: "A historical city with numerous monuments and relics from the Ahom dynasty."
  },
  {
    title: "Tezpur",
    image: "/images/Assam_img/Tezpur.jpg",
    description: "The principal attraction of Tezpur includes its rich history, old temples, forests, and wildlife."
  }
];

const OdishaCardData = [
  {
    title: "Bhubaneswar",
    image: "/images/Odisha_img/Bhubaneswar.jpg",
    description: "The capital city, Bhubaneswar is known for its ancient temples and rich cultural heritage."
  },
  {
    title: "Puri",
    image: "/images/Odisha_img/Puri.jpg",
    description: "Famous for the Jagannath Temple and its annual Rath Yatra, Puri is a significant pilgrimage destination."
  },
  {
    title: "Konark",
    image: "/images/Odisha_img/Konark.jpg",
    description: "Home to the Sun Temple, a UNESCO World Heritage Site, Konark is a must-visit for its architectural marvel."
  },
  {
    title: "Chilika Lake",
    image: "/images/Odisha_img/Chilika.jpg",
    description: "Asia's largest brackish water lagoon, Chilika Lake is a haven for bird watchers and nature lovers."
  },
  {
    title: "Cuttack",
    image: "/images/Odisha_img/Cuttack.jpg",
    description: "Known for its silver filigree work and the historic Barabati Fort, Cuttack is a city with rich heritage."
  }
];

const TamilNaduCardData = [
  {
    title: "Chennai",
    image: "/images/TamilNadu_img/Chennai.jpg",
    description: "The capital city, Chennai is known for its beaches, temples, and rich cultural heritage."
  },
  {
    title: "Madurai",
    image: "/images/TamilNadu_img/Madurai.jpg",
    description: "Famous for the Meenakshi Amman Temple, Madurai is an ancient city with a vibrant history."
  },
  {
    title: "Ooty",
    image: "/images/TamilNadu_img/Ooty.jpg",
    description: "A popular hill station in the Nilgiri Hills, Ooty is known for its tea gardens and picturesque landscapes."
  },
  {
    title: "Kanyakumari",
    image: "/images/TamilNadu_img/Kanyakumari.jpg",
    description: "The southernmost tip of India, Kanyakumari is famous for its beautiful sunrise and sunset views."
  },
  {
    title: "Mahabalipuram",
    image: "/images/TamilNadu_img/Mahabalipuram.jpg",
    description: "A UNESCO World Heritage Site, Mahabalipuram is renowned for its rock-cut temples and sculptures."
  }
];
 




const MPCardData = [
  {
    title: "Indore",
    image: "/images/Rajasthan_img/POHA.jpg",
    description: "Indore blends modernity with tradition: vibrant markets, delicious street food, and lively festivals."
  },
  {
    title: "Panchmari",
    image: "/images/Rajasthan_img/Panchmari.jpg",
    description: "Pachmarhi: serene hill station with lush forests, waterfalls, and tranquility."
  },
  {
    title: "Sanchi",
    image: "/images/Rajasthan_img/Sanchi.jpg",
    description: "Sanchi Stupa: ancient Buddhist monument, exquisite carvings, serene historical site."
  },
  {
    title: "Khajurao",
    image: "/images/Rajasthan_img/Khajurao.jpg",
    description: "Khajuraho: stunning temples, intricate carvings, rich history, and architectural marvels."
  },
  {
    title: "Kanha Park",
    image: "/images/Rajasthan_img/kanazone.jpg",
    description: "Kanha National Park: lush forests, diverse wildlife, and scenic landscapes."
  }
];
const MaharashtraCardData = [
  {
    title: "Mumbai",
    image: "/images/Rajasthan_img/mumba.jpg",
    description: "Mumbai: vibrant cityscape with iconic landmarks, bustling streets, and a lively cultural scene."
  },
  {
    title: "Lonavala",
    image: "/images/Rajasthan_img/Lonavala.jpg",
    description: "Lonavala: picturesque hill station known for its scenic views, lush greenery, and pleasant climate."
  },
  {
    title: "Aurangabad",
    image: "/images/Rajasthan_img/Aurangabad.jpg",
    description: "Aurangabad: historical city with ancient caves, including Ajanta and Ellora, rich cultural heritage."
  },
  {
    title: "Pune",
    image: "/images/Rajasthan_img/Pune.jpg",
    description: "Pune: vibrant cultural hub with historic forts, thriving arts scene, and lively student atmosphere."
  },
  {
    title: "Nagpur",
    image: "/images/Rajasthan_img/Nagpur.jpg",
    description: "Nagpur: known for its oranges, historical sites, and the nearby Tadoba National Park."
  }
];
const DamanCardData = [
  {
    title: "Daman",
    image: "/images/Rajasthan_img/de-daman.jpg",
    description: "Daman: coastal haven with serene beaches, historic forts, and charming colonial architecture."
  },
  {
    title: "Diu",
    image: "/images/Rajasthan_img/Diu-Fort.jpg",
    description: "Diu: tranquil island with pristine beaches, majestic Diu Fort, and picturesque St. Paul's Church."
  },
  {
    title: "J.Beach",
    image: "/images/Rajasthan_img/jampore-beach.jpg",
    description: "Jampore Beach: peaceful beach ideal for relaxation, water sports, and picturesque sunsets."
  },
  {
    title: "N.Beach",
    image: "/images/Rajasthan_img/Nagoa-Beachjpg.jpg",
    description: "Nagoa Beach: popular beach destination offering clear waters, coconut groves, and scenic beauty."
  },
  {
    title: "Jerome Fort",
    image: "/images/Rajasthan_img/Jerome-Fort-In-Daman.jpg",
    description: "St. Jerome Fort: historic fortification with panoramic views, rich history, and Portuguese influence."
  }
];
const GoaCardData = [
  {
    title: "Baga Beach",
    image: "/images/Rajasthan_img/Bagabeach.jpg",
    description: "Baga Beach: lively beach known for its vibrant nightlife, water sports, and bustling shacks."
  },
  {
    title: "Old Goa",
    image:"/images/Rajasthan_img/oldgoa.jpg",
    description: "Old Goa: historical area with magnificent churches and cathedrals reflecting Portuguese heritage."
  },
  {
    title: "Palolem Beach",
    image: "/images/Rajasthan_img/palobeach.jpg",
    description: "Palolem Beach: picturesque beach famous for its calm waters, scenic beauty, and beach huts."
  },
  {
    title: "Dudhsagar Falls",
    image: "/images/Rajasthan_img/dhudhsagar.jpg",
    description: "Dudhsagar Falls: majestic four-tiered waterfall set amidst lush greenery, perfect for nature lovers."
  },
  {
    title: "Aguada Fort",
    image: "/images/Rajasthan_img/Fort-Aguada-aerial-view.jpg",
    description: "Aguada Fort: 17th-century Portuguese fort offering panoramic views of the Arabian Sea."
  }
];

const KeralaCardData = [
  {
    title: "Backwaters",
    image:  "/images/Rajasthan_img/blackwater.jpg",
    description: "Backwaters: serene network of waterways offering tranquil houseboat experiences and lush landscapes."
  },
  {
    title: "Munnar",
    image: "/images/Rajasthan_img/munnar.jpg",
    description: "Munnar: scenic hill station known for its tea plantations, lush greenery, and panoramic views."
  },
  {
    title: "Alleppey",
    image: "/images/Rajasthan_img/alleppey.jpg",
    description: "Alleppey: charming town famous for its houseboats and tranquil backwaters."
  },
  {
    title: "Eravikulam ",
    image: "/images/Rajasthan_img/eravikulam-national-park.jpg",
    description: "Eravikulam National Park: wildlife sanctuary renowned for its diverse flora and fauna, including the Nilgiri Tahr."
  },
  {
    title: "Kochi",
    image: "/images/Rajasthan_img/kochi.jpg",
    description: "Kochi: vibrant city with historical forts, cultural heritage sites, and a lively waterfront."
  }
];

const KarnatakaCardData = [
  {
    title: "Coorg",
    image: "/images/Rajasthan_img/coorg.jpg",
    description: "Coorg: picturesque hill station known for its coffee plantations, lush greenery, and pleasant climate."
  },
  {
    title: "Mysore",
    image: "/images/Rajasthan_img/MYSOREkarna.jpg",
    description: "Mysore: historic city famed for its palaces, especially the grand Mysore Palace, and vibrant Dasara festival."
  },
  {
    title: "Hampi",
    image: "/images/Rajasthan_img/hampi.jpg",
    description: "Hampi: UNESCO World Heritage Site with ancient temples, ruins, and unique rock formations."
  },
  {
    title: "Bangalore",
    image: "/images/Rajasthan_img/bango.jpg",
    description: "Bangalore: the cosmopolitan capital city known for its parks, nightlife, and tech industry."
  },
  {
    title: "Bandipur",
    image: "/images/Rajasthan_img/bandipur.jpg",
    description: "Bandipur National Park: renowned wildlife reserve home to tigers, elephants, and a variety of flora and fauna."
  }
];


const AndhraPradeshCardData = [
  {
    title: "Visakhapatnam",
    image: "/images/Rajasthan_img/vishu.jpg",
    description: "Visakhapatnam: coastal city known for its beautiful beaches, scenic hills, and historic sites."
  },
  {
    title: "Tirupati",
    image: "/images/Rajasthan_img/tirupati.jpg",
    description: "Tirupati: famous pilgrimage city home to the Tirumala Venkateswara Temple, one of the richest and most visited temples in the world."
  },
  {
    title: "Araku Valley",
    image: "/images/Rajasthan_img/Arakuvalley.jpg",
    description: "Araku Valley: picturesque hill station known for its coffee plantations, lush greenery, and tribal culture."
  },
  {
    title: "Vijayawada",
    image: "/images/Rajasthan_img/vijaywada.jpg",
    description: "Vijayawada: vibrant city on the banks of the Krishna River, known for the Kanaka Durga Temple and Prakasam Barrage."
  },
  {
    title: "Lepakshi",
    image: "/images/Rajasthan_img/leepakshi.jpg",
    description: "Lepakshi: historic village famous for its Veerabhadra Temple, intricate sculptures, and hanging pillar."
  }
];
const MeghalayaCardData = [
  {
    title: "Shillong",
    image: "/images/Rajasthan_img/shilong.jpg",
    description: "Shillong: the capital city known for its picturesque landscapes, vibrant culture, and colonial architecture."
  },
  {
    title: "Cherrapunji",
    image: "/images/Rajasthan_img/shillong-cherrapunji.jpg",
    description: "Cherrapunji: renowned for being one of the wettest places on earth, famous for its living root bridges and stunning waterfalls."
  },
  {
    title: "Mawlynnong",
    image: "/images/Rajasthan_img/living-root-bridge.jpg",
    description: "Mawlynnong: known as the 'Cleanest Village in Asia', famous for its cleanliness, living root bridges, and natural beauty."
  },
  {
    title: "Dawki",
    image: "/images/Rajasthan_img/Dawki-Meghalaya-Tourist-Cabs.jpg",
    description: "Dawki: a small town known for its crystal clear Umngot River and beautiful landscapes."
  },
  {
    title: "Nongriat",
    image: "/images/Rajasthan_img/nongriat.jpg",
    description: "Nongriat: famous for its double-decker living root bridge and pristine natural environment."
  }
];

const UttarPradeshCardData = [
  {
    title: "Agra",
    image: " /images/Rajasthan_img/taj-mahal.jpg",
    description: "Agra: home to the iconic Taj Mahal, Agra Fort, and Fatehpur Sikri."
  },
  {
    title: "Varanasi",
    image: "/images/Rajasthan_img/varnasi.jpg",
    description: "Varanasi: one of the oldest living cities, known for its ghats, temples, and vibrant spiritual culture."
  },
  {
    title: "Lucknow",
    image: "/images/Rajasthan_img/lucka.jpg",
    description: "Lucknow: the City of Nawabs, famous for its Mughal architecture, cuisine, and vibrant arts scene."
  },
  {
    title: "Allahabad",
    image: "/images/Rajasthan_img/allahabad.jpg",
    description: "Allahabad: known for the Triveni Sangam, where the Ganges, Yamuna, and mythical Saraswati rivers meet."
  },
  {
    title: "Mathura",
    image: "/images/Rajasthan_img/mathura,jpg.jpg",
    description: "Mathura: the birthplace of Lord Krishna, renowned for its temples and festive celebrations."
  }
];



 
  export { RajasthanCardData, GujratCardData , MPCardData, MaharashtraCardData, DamanCardData, GoaCardData,KeralaCardData , KarnatakaCardData, AndhraPradeshCardData, MeghalayaCardData, UttarPradeshCardData ,LadakhCardData, JammuAndKashmirCardData, HimachalPradeshCardData, UttarakhandCardData, SikkimCardData, WestBengalCardData, AssamCardData, OdishaCardData, TamilNaduCardData  };
